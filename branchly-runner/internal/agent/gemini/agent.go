...
func (a *Agent) Run(ctx context.Context, input domain.AgentInput) (string, error) {
	...
	if err := cmd.Wait(); err != nil {
		...
		stderrMu.Unlock()
		if tail == "" {
			tail = "(no stderr; check GEMINI_API_KEY is set in the runner environment)"
		}
		if s != "" {
			return s, fmt.Errorf("gemini exited: %w — %s", err, tail)
		}
		return "", fmt.Errorf("gemini exited: %w — %s", err, tail)
	}
	lineMu.Lock()
	defer lineMu.Unlock()
	return lastLine, nil
